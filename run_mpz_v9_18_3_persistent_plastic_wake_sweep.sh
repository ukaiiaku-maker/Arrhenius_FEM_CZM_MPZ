...
OUTROOT_BASE=${OUTROOT_BASE:-runs/mpz_v9_18_3_edge_aware_geometry_sweep_v1}
TEMPS=${TEMPS:-"700"}
SEEDS=${SEEDS:-"1"}
...
GEOMETRY_BARYCENTRIC_EDGE_TOL=${GEOMETRY_BARYCENTRIC_EDGE_TOL:-1e-8}
...
for T in $TEMPS; do
  ...
  mkdir -p "$case_root"
  echo "=== v9.18.3 edge-aware geometry: T=${T} K, committed target=${TARGET_EXT_UM} um ==="
  set +e
  ...
  ARRHENIUS_GEOMETRY_BARYCENTRIC_EDGE_TOL="$GEOMETRY_BARYCENTRIC_EDGE_TOL" \
  ...
  "$PYTHON_BIN" run_mpz_v9_18_3_persistent_plastic_wake.py \
