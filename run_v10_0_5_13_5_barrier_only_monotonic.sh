...
ALLOW_OUTROOT_MODE_MISMATCH=${ALLOW_OUTROOT_MODE_MISMATCH:-0}

# Rate-preserving adaptive macro-step.
...
case "$MODE" in
  smoke)
    DEFAULT_OUTROOT="$ROOT_DIR/runs/v10_0_5_13_5_tip_only_DBTT_700K_20um_macro100_v1"
    ;;
  full)
    DEFAULT_OUTROOT="$ROOT_DIR/runs/v10_0_5_13_5_tip_only_4class_300_1200K_100um_macro100_v1"
    ;;
  *)
...
if [[ "$ALLOW_OUTROOT_MODE_MISMATCH" != "1" ]]; then
  case "$MODE:$OUTROOT" in
    full:*smoke*|full:*DBTT_700K_20um*|smoke:*4class*|smoke:*300_1200K_100um*)
      echo "ERROR: OUTROOT appears inconsistent with MODE=$MODE:" >&2
      echo "  $OUTROOT" >&2
      echo "Unset OUTROOT or choose a new mode-specific output directory." >&2
      exit 2
      ;;
  esac
fi
...
args=(
  "$PYTHON_BIN" "$ROOT_DIR/run_v10_0_5_13_5_barrier_only_monotonic.py"
  --mode "$MODE"
...
printf 'Python: %s\n' "$PYTHON_BIN"
printf 'Output: %s\n' "$OUTROOT"
printf 'Point release: 10.0.5.13.5\n'
printf 'Plasticity scope: tip_only (elastic bulk + moving crack-tip MPZ)\n'
printf 'Long-corridor policy: expanded deterministic center-count search; quality floor retained\n'
printf 'Ramp audit: %s\n' "$RATE_AUDIT"
printf 'Launching:'
printf ' %q' "${args[@]}"
printf '\n'
