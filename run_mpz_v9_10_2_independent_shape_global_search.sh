...
OUTROOT="${OUTROOT:-runs/mpz_v9_10_2_independent_shape_global_search_v1}"
...
run_one() {
  local class_name="$1"
  local class_seed="$2"
  echo "[$(date '+%Y-%m-%d %H:%M:%S')] starting v9.10.2 independent-shape class=$class_name"
  conda run -n "$CONDA_ENV" --no-capture-output "$PYTHON_BIN" -u \
    ...
    --da-um "$DA_UM" \
    --out "$OUTROOT"
  echo "[$(date '+%Y-%m-%d %H:%M:%S')] completed v9.10.2 independent-shape class=$class_name"
}
...
echo "[$(date '+%Y-%m-%d %H:%M:%S')] all v9.10.2 independent-shape searches complete"
