...
CAMPAIGN_ROOT=${CAMPAIGN_ROOT:-$ROOT/runs/v10_0_5_18_4_0_peak1000_theta0_seed8666_pf_numeric_parity_v1}
...
do
  if [[ ! -f "$path" ]]; then
    echo "ERROR: required parity input is missing: $path" >&2
    exit 1
  fi
done
...
if [[ "$FAMILY_SHA" != "$FAMILY_SHA_REQUIRED" ]]; then
  echo "ERROR: PF theta=0 signed-kernel family SHA-256 mismatch" >&2
...
entry=arrhenius_fracture.mode_i_first_passage_v10_0_5_18_4_0_theta0_pf_parity
reference_case=$PF_CASE
reference_model=PF_v10.4.1_bulk_detailed_balance
parameter_option=$OPTION
temperature_K=$TEMPERATURE
theta_deg=$THETA
hazard_seed=$CASE_SEED
target_projected_extension_um=$TARGET_EXT_UM
nx=36
ny=72
dU_m=2e-7
dt_s=8.4
opening_rate_m_per_s=2.3809523809523807e-8
n_stagger=2
tip_h_fine_m=1e-6
tip_ratio=1.20
da_phys_m=5e-6
...
echo "[START] option=$OPTION T=${TEMPERATURE}K theta=$THETA seed=$CASE_SEED"
...
env \
  ...
  python -u -m arrhenius_fracture.mode_i_first_passage_v10_0_5_18_4_0_theta0_pf_parity \
...
echo "[DONE] option=$OPTION T=${TEMPERATURE}K theta=$THETA seed=$CASE_SEED"
