#!/usr/bin/env python3
"""One-run-per-condition v3.4 campaign runner for the validated v3.3 mechanics."""
...
def main():
    ...
    p.add_argument('--T-K',type=float,default=500);p.add_argument('--outroot',default='runs/mixed_mode_fem_czm_v3_4_campaign_500K')
    ...
    pd.DataFrame(results).to_csv(outroot/'campaign_status_v3_4.csv',index=False)
    good=[r for r in results if r['status']!='failed']
    if good:pd.DataFrame(good).to_csv(outroot/'mixed_mode_v3_4_all_cases.csv',index=False)
