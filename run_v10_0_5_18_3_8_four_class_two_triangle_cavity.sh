...
TMP=$(mktemp "${TMPDIR:-/tmp}/v10051838_two_triangle_cavity.XXXXXX")
trap 'rm -f "$TMP"' EXIT
sed \
  -e 's/mode_i_first_passage_v10_0_5_18_3_4_four_class_path_aware_growth_envelope/mode_i_first_passage_v10_0_5_18_3_8_two_triangle_cavity/g' \
  ...
  -e 's/Focused v10.0.5.18.3.4 path-envelope validation complete/Focused v10.0.5.18.3.8 two-triangle cavity validation complete/g' \
  "$BASE_RUNNER" > "$TMP"
chmod +x "$TMP"
bash "$TMP"
