#!/usr/bin/env python3
"""Run long-growth Mode-I v9.11 FEM/CZM R-curves at selected temperatures.

The historical filename is retained for compatibility. The driver now also
selects an explicit bulk mode:

* ``tip_only``: elastic continuum bulk, moving crack-tip MPZ is the only source;
* ``bulk_same_pt_km``: explicit bulk mobile/retained state using the same class
  Peierls/Taylor manifest and the repository's existing Kocks--Mecking law.
"""
...
from arrhenius_fracture.bulk_state_v911 import normalize_bulk_mode
...
def observed_K_init(fp: dict) -> float | None:
    """Return an initiation value only when first passage was actually observed."""
    control = str(fp.get("control_state", "")).strip().lower()
    existing = fp.get("Kc_first_existing_MPa_sqrt_m")
    observed = control == "first_passage" or existing not in (None, "", "nan")
    if not observed:
        return None
    value = fp.get("KJ_reference_first_MPa_sqrt_m", existing)
    try:
        return float(value)
...
def endpoint_reference(fp: dict) -> float | None:
    value = fp.get("KJ_reference_first_MPa_sqrt_m")
    try:
        return float(value)
    except (TypeError, ValueError):
        return None
...
def build_command(py: str, args, class_name: str, manifest: Path, T_K: int, case_dir: Path) -> list[str]:
    cmd = [
        ...
        "--bulk-plasticity-mode", normalize_bulk_mode(getattr(args, "bulk_plasticity_mode", "tip_only")),
...
def read_first_passage(case_dir: Path) -> dict:
    return read_json(case_dir / "anisotropic_calibrated_tip_first_passage_summary.json")


def read_bulk_summary(case_dir: Path) -> dict:
    return read_json(case_dir / "bulk_state_v9_11_summary.json")


def run_case(py: str, args, class_name: str, manifest: Path, T_K: int, root: Path) -> dict:
    mode = normalize_bulk_mode(args.bulk_plasticity_mode)
    ...
    if complete and args.skip_existing:
        ...
        bulk = read_bulk_summary(case_dir)
        Kinit = observed_K_init(fp)
        row = {
            "class": class_name,
            ...
            "K_endpoint_reference_MPa_sqrt_m": endpoint_reference(fp),
            **bulk,
            **propagation_metrics(rc, Kinit),
        }
        print(f"SKIP {class_name} {mode} T={T_K} K: extension={extension_um:.1f} um, events={len(rc)}")
        return row
    ...
    print(f"START {class_name} {mode} T={T_K} K -> {case_dir}")
    ...
    row = {
        ...
        "K_init_MPa_sqrt_m": Kinit,
        "K_endpoint_reference_MPa_sqrt_m": endpoint_reference(fp),
        "control_state": fp.get("control_state"),
    ...
    print(
        f"{status.upper():14s} {class_name} {mode} T={T_K} K "
        f"extension={extension_um} um, events={len(rc)}, Kinit={Kinit}"
    )
    return row
...
def main() -> None:
    ...
    p.add_argument("--bulk-plasticity-mode", default="tip_only", choices=("tip_only", "bulk_same_pt_km"))
    ...
    args.bulk_plasticity_mode = normalize_bulk_mode(args.bulk_plasticity_mode)
    temps = [int(round(x)) for x in values(args.temperatures, float)]
    if not temps:
        raise SystemExit("--temperatures must contain at least one temperature")
    ...
    frame.to_csv(root / "rcurve_temperature_summary.csv", index=False)
    ...
    incomplete = frame[~frame["status"].isin(["complete", "skipped_complete"])]
    print(frame.to_string(index=False))
    print("wrote", root / "rcurve_temperature_summary.csv")
    if not incomplete.empty:
        raise SystemExit(
            "one or more R-curve cases did not reach the requested extension; "
            "inspect the per-case run.log and bulk_state_v9_11_summary.json"
        )
