...
CAMPAIGN_ROOT=${CAMPAIGN_ROOT:-$ROOT/runs/v10_0_5_18_3_2_four_class_focused_100um_theta${THETA:-30}_v1}
...
TEMPERATURES=${TEMPERATURES:-"300 1000"}
THETA=${THETA:-30}
...
EMISSION_RAMP_MAX_LOG_RATE_CHANGE=${EMISSION_RAMP_MAX_LOG_RATE_CHANGE:-0.25}
EMISSION_RAMP_ACTION_FLOOR=${EMISSION_RAMP_ACTION_FLOOR:-1e-6}
EMISSION_INNER_MAX_LOG_HAZARD_CHANGE=${EMISSION_INNER_MAX_LOG_HAZARD_CHANGE:-0.25}
EMISSION_INNER_ACTION_FLOOR=${EMISSION_INNER_ACTION_FLOOR:-1e-6}
EMISSION_EVENT_HORIZON_FACTOR=${EMISSION_EVENT_HORIZON_FACTOR:-1.25}
EMISSION_INNER_MAX_REFINEMENTS=${EMISSION_INNER_MAX_REFINEMENTS:-24}
...
with Path(os.environ["SEED_MAP"]).open("w", newline="") as stream:
    ...
    writer.writerow([
        "parameter_option", "temperature_K", "cleavage_case_seed",
        "emission_channel_0_seed", "emission_channel_1_seed",
        "replicate", "theta_deg",
    ])
...
release=10.0.5.18.3.2
entry=arrhenius_fracture.mode_i_first_passage_v10_0_5_18_3_2_four_class_joint_K_ramp_stochastic_emission
...
emission_hazard_drive_quadrature=linear_K_endpoint_trapezoid
emission_transport_drive_quadrature=midpoint_opening_stress_on_each_adaptive_horizon
...
run_case() {
  ...
  local MANIFEST="$OUT/persistent_site_production_manifest_v10_0_5_18_3_2.json"
  ...
  env \
    ...
    EMISSION_INNER_MAX_LOG_HAZARD_CHANGE="$EMISSION_INNER_MAX_LOG_HAZARD_CHANGE" \
    EMISSION_INNER_ACTION_FLOOR="$EMISSION_INNER_ACTION_FLOOR" \
    EMISSION_EVENT_HORIZON_FACTOR="$EMISSION_EVENT_HORIZON_FACTOR" \
    EMISSION_INNER_MAX_REFINEMENTS="$EMISSION_INNER_MAX_REFINEMENTS" \
    TWO_CHANNEL_PROBE_FALLBACK_MAX_CALLS="$TWO_CHANNEL_PROBE_FALLBACK_MAX_CALLS" \
    python -u -m arrhenius_fracture.mode_i_first_passage_v10_0_5_18_3_2_four_class_joint_K_ramp_stochastic_emission \
...
export EMISSION_INNER_MAX_LOG_HAZARD_CHANGE EMISSION_INNER_ACTION_FLOOR
export EMISSION_EVENT_HORIZON_FACTOR EMISSION_INNER_MAX_REFINEMENTS
...
echo "Focused v10.0.5.18.3.2 validation complete: $CAMPAIGN_ROOT"
