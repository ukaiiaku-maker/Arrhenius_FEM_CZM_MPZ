...
with
    ...
    F_T(T) = hill(T/T_S,p_T) / hill(T0/T_S,p_T)
    F_sigma(sigma) = hill(|sigma|/sigma_S,p_sigma)

All user-facing stress parameters are expressed in MPa.

The derived stress-entropy coordinate is

    DeltaG_S,sigma,ref = -T0 [S*(sigma_ref,T0)-S*(0,T0)]
                       = kB T0 A_sigma F_sigma(sigma_ref)

    Lambda_S,ref = DeltaG_S,sigma,ref / H0

This is a diagnostic derived from the entropy function, not an independent
shielding parameter.

The common parent barrier is
...
class EntropyFamily:
    ...
    A_T_kB: float = 4.0
    A_sigma_kB: float = 8.0
    T0_K: float = 300.0
    T_S_K: float = 400.0
    T_gate_power: float = 4.0
    sigma_S_MPa: float = 400.0
    sigma_gate_power: float = 6.0
    S_min_kB: float = -80.0
    S_max_kB: float = 0.0
    ...
    def stress_gate(self, sigma_Pa):
        ...
        s0 = max(float(self.sigma_S_MPa) * 1e6, 1.0)
    ...
    def deltaG_stress_entropy_ref_eV(self, sigma_ref_MPa: float, T_ref_K: float | None = None) -> float:
        """Stress-dependent entropic barrier increment at a physical reference state.

        Returns -T [S(sigma,T)-S(0,T)] in eV.  The value is positive for the
        present negative stress-dependent entropy contribution.
        """
        T = float(self.T0_K if T_ref_K is None else T_ref_K)
        sref = float(sigma_ref_MPa) * 1e6
        S0 = float(np.asarray(self.S_kB(np.array([0.0]), T))[0])
        Ss = float(np.asarray(self.S_kB(np.array([sref]), T))[0])
        return -KB_EV_PER_K * T * (Ss - S0)


@dataclass(frozen=True)
class ParentArrheniusBarrier:
    """Common H - T*S - sigma*v barrier used by Panels C and D."""

    H0_eV: float = 0.8
    sigma0_H_MPa: float = 2500.0
    v0_b3: float = 0.6
    sigma0_v_MPa: float = 2500.0
    b_m: float = 2.74e-10
    entropy: EntropyFamily = EntropyFamily()
    monotone_stress: bool = True
    monotone_search_MPa: float = 100000.0
    monotone_grid_n: int = 4001

    def H_J(self, sigma_Pa):
        sigma = np.abs(np.asarray(sigma_Pa, dtype=float))
        s0 = max(float(self.sigma0_H_MPa) * 1e6, 1.0)
        return float(self.H0_eV) * EV_TO_J / (1.0 + (sigma / s0) ** 2)

    def v_m3(self, sigma_Pa):
        sigma = np.abs(np.asarray(sigma_Pa, dtype=float))
        s0 = max(float(self.sigma0_v_MPa) * 1e6, 1.0)
    ...
    def _sigma_argmin_scalar(self, T_key: float) -> float:
        sg = np.linspace(
            0.0,
            max(float(self.monotone_search_MPa), 1e-6) * 1e6,
    ...
    def lambda_S_ref(self, sigma_ref_MPa: float, T_ref_K: float | None = None) -> float:
        dG = self.entropy.deltaG_stress_entropy_ref_eV(sigma_ref_MPa, T_ref_K)
        return dG / max(float(self.H0_eV), 1e-30)
...
def build_parent_barrier(
    ...
    *,
    ...
    return ParentArrheniusBarrier(
        H0_eV=float(H0_eV),
        sigma0_H_MPa=float(sigma0_H_MPa),
        v0_b3=float(v0_b3),
        sigma0_v_MPa=float(sigma0_v_MPa),
        b_m=float(b_m),
        entropy=ent,
