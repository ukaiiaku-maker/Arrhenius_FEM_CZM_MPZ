#!/usr/bin/env python3
"""Fail-closed audit for the first 5 um progressive kinetic-CZM event."""
...
SCHEMA = "kinetic_campaign_czm_progressive_smoke_v10_0"
...
def audit(root: Path, target_um: float = 5.0) -> dict[str, Any]:
    root = Path(root)
    progressive_path = find_one(root, "kinetic_campaign_czm_progressive_2d_v10_0.json")
    ...
    require(progressive.get("full_progressive_trial_loop_active") is True,
            "dedicated progressive loop was not active")
    ...
    require(int(progressive.get("damage_rejections", 0)) == 0,
            "outer trial-damage increment was rejected")
    require(float(progressive.get("mpz_advance_on_commit_m", math.nan)) == 0.0,
            "MPZ was translated again at outer commitment")
    require(progressive.get("one_topology_event_per_geometry_solve") is True,
            "one-event topology gate is not active")
    ...
    require(nondecreasing(damages), "trial cohesive damage is not monotonic")
    require(math.isclose(damages[-1], 1.0, rel_tol=0.0, abs_tol=1.0e-10),
            f"final trial damage is not one: {damages[-1]}")
    require(bool(records[-1].get("fired", False)), "final progressive record did not commit")
    require(sum(bool(row.get("fired", False)) for row in records) == 1,
            "more than one committed checkpoint appears in the progressive records")
    ...
    for row in accepted:
        ...
        require(not row.get("issues", []), f"accepted quality row has issues: {row.get('issues')}")
    ...
    log_event_ids = {int(row.get("physical_event_index", row.get("event_index", -1))) for row in committed}
    ...
    payload = {
        ...
        "damage_monotonic": True,
    }
    out = root / "progressive_one_segment_smoke_v10_0.json"
