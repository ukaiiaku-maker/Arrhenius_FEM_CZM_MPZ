...
def _adjacent_target_triangle(self: Any, mesh: Any, p0: np.ndarray, target: np.ndarray, front_id: int):
    ...
    if len(incident) == 0:
        return None, 0
    fan_nodes = set(map(int, np.asarray(mesh.elems, dtype=int)[np.asarray(incident, dtype=int)].ravel()))
    target_nodes = set(map(int, np.asarray(mesh.elems[int(elem_id)], dtype=int)))
...
def _ray_handoff_candidate(self: Any, state: _base.State, p0: np.ndarray, target: np.ndarray, front_id: int, level: int):
    elem_id = _base._target_tip_triangle(self, state.mesh, p0, target, front_id)
    if elem_id is None:
        return None, {"level": int(level), "accepted": False, "reason": "no_target_tip_triangle"}
    tip_node = _base._tip_node_in_triangle(state.mesh, elem_id, p0)
    if tip_node is None:
        return None, {
            "level": int(level), "accepted": False,
            "reason": "target_triangle_has_no_tip_vertex",
            "target_parent_element": int(elem_id),
        }
    qfloor = _base.float_env("ARRHENIUS_MIN_ACCEPTED_TRIANGLE_QUALITY", float(self.min_triangle_quality))
    afloor = _base.float_env("ARRHENIUS_MIN_ACCEPTED_CHILD_AREA_RATIO", float(self.min_area_ratio))
    ray = np.asarray(target, dtype=float) - np.asarray(p0, dtype=float)
    ray_norm = float(np.linalg.norm(ray))
    if ray_norm <= 0.0:
        return None, {"level": int(level), "accepted": False, "reason": "zero_length_ray_handoff"}
    ...
    for edge_i, edge_j in radial_edges:
        candidate_state, split = _base._midpoint_split_candidate(self, state, edge_i, edge_j, qfloor, afloor)
        if candidate_state is None:
            errors.append(split)
            continue
        candidate_elem = _base._target_tip_triangle(self, candidate_state.mesh, p0, target, front_id)
        if candidate_elem is not None:
            continue
        adjacent_elem, shared = _adjacent_target_triangle(self, candidate_state.mesh, p0, target, front_id)
        if adjacent_elem is None:
            errors.append({**split, "reason": "split_target_not_adjacent_to_tip_fan"})
            continue
        ...
        if vector_norm <= 0.0 or forward <= 0.0:
            errors.append({**split, "reason": "ray_handoff_midpoint_not_forward", "ray_forward_projection_m": forward})
            continue
        perpendicular = float(np.linalg.norm(vector - forward * ray_unit))
        alignment = float(np.clip(forward / vector_norm, -1.0, 1.0))
        qmargin = float(split["min_triangle_quality"]) / max(qfloor, 1.0e-300)
        amargin = float(split["min_immediate_child_area_ratio"]) / max(afloor, 1.0e-300)
        score = (alignment, -perpendicular, min(qmargin, 10.0), min(amargin, 10.0), -int(edge_j))
        candidates.append((score, candidate_state, {
            **split,
            "level": int(level),
            "accepted": True,
            "refinement_kind": "endpoint_centered_tip_radial_ray_handoff",
...
def _target_cavity_candidate(self: Any, state: _base.State, p0: np.ndarray, target: np.ndarray, front_id: int, level: int):
    elem_id, shared = _adjacent_target_triangle(self, state.mesh, p0, target, front_id)
    if elem_id is None:
        return None, {"level": int(level), "accepted": False, "reason": "no_adjacent_ray_target_triangle"}
    qfloor = _base.float_env("ARRHENIUS_MIN_ACCEPTED_TRIANGLE_QUALITY", float(self.min_triangle_quality))
    afloor = _base.float_env("ARRHENIUS_MIN_ACCEPTED_CHILD_AREA_RATIO", float(self.min_area_ratio))
    conn = [int(value) for value in state.mesh.elems[int(elem_id)]]
    edges = sorted(set([
    ...
    for edge_i, edge_j in edges:
        ...
        candidate_elem, candidate_shared = _adjacent_target_triangle(
            self, candidate_state.mesh, p0, target, front_id
        )
        if candidate_elem is None:
            errors.append({**split, "reason": "target_cavity_lost_tip_fan_adjacency"})
            continue
        predicted = _base._predicted_exact_target_metrics(
            self, candidate_state.mesh, candidate_elem, np.asarray(target, dtype=float)
        )
        if predicted is None:
            errors.append({**split, "reason": "cannot_predict_target_cavity_insertion"})
            continue
        ...
        score = (
            int(exact_pass), int(candidate_shared), float(limiting),
            float(min(qmargin, 10.0)), float(min(amargin, 10.0)),
            -int(edge_j), -int(edge_i),
        )
        candidates.append((score, candidate_state, {
            **split, **predicted,
            "level": int(level),
...
def refine_once(self: Any, state: _base.State, p0: np.ndarray, target: np.ndarray, front_id: int, level: int):
    refined, record = _ORIGINAL_REFINE_ONCE(self, state, p0, target, front_id, level)
    ...
    if reason == "no_quality_safe_tip_radial_midpoint_bisection":
        handoff_state, handoff = _ray_handoff_candidate(
            self, state, np.asarray(p0, dtype=float), np.asarray(target, dtype=float), int(front_id), int(level)
        )
