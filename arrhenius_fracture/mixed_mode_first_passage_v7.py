...
V7 preserves the v5 calibrated-tip correction and adds exact production-backend loading control.  The existing sharp-front/CZM engine
...
MODEL_ID = "FEM_CZM_mixed_mode_first_passage_v7_exact_backend_full_circle"
...
def main(argv=None):
    ...
    if args.mode != "2d":
        raise SystemExit("v7 anisotropic mixed mode requires --mode 2d")
    if not bool(getattr(args, "crystal_aniso", False)):
        raise SystemExit("v7 requires --crystal-aniso")
    if not bool(getattr(args, "crystal_compete", False)):
        raise SystemExit("v7 requires --crystal-compete")
    if bool(getattr(args, "crystal_branch", False)) or int(getattr(args, "max_fronts", 1)) != 1:
        raise SystemExit("v7 first-passage screen requires branching off and --max-fronts 1")
    ...
    print("MIXED_MODE_V7 complete:", json.dumps(vals, indent=2, default=str))
    return vals
