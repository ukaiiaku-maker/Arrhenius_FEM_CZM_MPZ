...
class CalibratedTipEngineMixin:
    ...
    def _sigma_from_drive(self, Kdrive):
        if getattr(self, 'state_model', 'legacy_scalar') == 'moving_pz':
            return float(self.sigma_tip(Kdrive))
    ...
    def predict_clock_increment(self, K, T, dt):
        ...
        if (getattr(self, 'state_model', 'legacy_scalar') == 'moving_pz'
                and hasattr(self, 'predict_clock_increment_drives')):
    ...
    def step(self, K, T, dt):
        ...
        if (getattr(self, 'state_model', 'legacy_scalar') == 'moving_pz'
            ...
            return self.step_drives(Kc, Ke, T, dt, metadata=md)
...
def _engine_factory(original_build, context, base_class):
    def build(args, mat):
        ...
        # Preserve the actual engine type returned by build_engine (legacy or
        # moving-PZ) and add the calibrated mixed-mode behavior in place.
        Engine = type('CalibratedMixedModeEngine',
                      (CalibratedTipEngineMixin, base.__class__), {})
        base.__class__ = Engine
        base._mm_init(context)
        return base
    return build
...
def _float_or_nan(value):
    """Convert optional diagnostic values to float without failing on None."""
    try:
...
def _summary(out, T, context, base_summary):
    ...
    KJ = _float_or_nan(rec.get("KJ_reference_Pa_sqrt_m", np.nan))
    fc = _float_or_nan(rec.get("cleavage_factor", np.nan))
    fe = _float_or_nan(rec.get("emission_factor", np.nan))
    payload = {
        "model": MODEL_ID,
        "T_K": float(T),
        "loading_angle_deg": context.loading_angle_deg,
        ...
        "reference_cleavage_shape": context.reference_cleavage_shape,
        "reference_slip_shape": context.reference_slip_shape,
        "reference_sigma_nn_first_GPa": _float_or_nan(rec.get("reference_sigma_nn_Pa", np.nan))/1e9,
        "reference_tau_tn_first_GPa": _float_or_nan(rec.get("reference_tau_tn_Pa", np.nan))/1e9,
        "probe_sigma1_first_GPa": _float_or_nan(rec.get("probe_sigma1_Pa", np.nan))/1e9,
        "candidate_sigma_nn_first_GPa": _float_or_nan(rec.get("candidate_sigma_nn_Pa", np.nan))/1e9,
        "candidate_overdrive_first_GPa": _float_or_nan(rec.get("candidate_overdrive_Pa", np.nan))/1e9,
        "slip_tau_abs_first_GPa": _float_or_nan(rec.get("slip_tau_abs_Pa", np.nan))/1e9,
