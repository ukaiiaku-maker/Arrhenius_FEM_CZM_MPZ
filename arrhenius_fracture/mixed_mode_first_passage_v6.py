...
V6 preserves the v5 calibrated-tip correction and adds exact production-backend loading control.  The existing sharp-front/CZM engine
...
MODEL_ID = "FEM_CZM_mixed_mode_first_passage_v6_production_backend_control"
...
def loading_coefficients_from_response_basis(response_matrix, target_phase_deg):
    ...
    p = math.radians(float(target_phase_deg))
    q = np.linalg.solve(M, np.array([math.cos(p), math.sin(p)]))
    qo, qs = normalize_loading_coefficients(q[0], q[1])
    r = M @ np.array([qo, qs])
    if r[0] < 0.0:
        qo, qs = -qo, -qs
    return qo, qs
...
def loading_z_from_coefficients(open_coeff, shear_coeff, open_floor=1e-12):
    qo, qs = normalize_loading_coefficients(open_coeff, shear_coeff)
    if qo <= 0.0:
        # The controlled mixed-mode range in this campaign has positive opening.
        qo, qs = -qo, -qs
    ratio = qs / max(qo, float(open_floor))
    return float(np.arcsinh(ratio))


def loading_coefficients_from_z(z):
    ratio = float(np.sinh(float(z)))
    return normalize_loading_coefficients(1.0, ratio)


def phase_from_response_z(response_matrix, z, shear_sign=1.0):
    qo, qs = loading_coefficients_from_z(z)
    r = np.asarray(response_matrix, float) @ np.array([qo, qs])
    return traction_phase_deg(r[0], r[1], shear_sign)


def safeguarded_event_z_update(samples, target_phase_deg, response_matrix,
                               max_step=1.5, z_bound=14.0):
    """Empirical event-state root update in an unconstrained shear/open coordinate.

    `samples` is a sequence of dictionaries containing finite `loading_z` and
    `achieved_psi_deg`.  A bracketed secant step is preferred.  Before a bracket
    exists, an empirical secant is used; for the first sample the exact
    production-backend elastic response matrix supplies only the derivative sign
    and scale.  This avoids the artificial +/-89.9 degree saturation of v5.
    """
    valid = []
    for row in samples:
        z = float(row.get("loading_z", np.nan))
        ph = float(row.get("achieved_psi_deg", np.nan))
        if np.isfinite(z) and np.isfinite(ph):
            valid.append((z, angle_error_deg(ph, target_phase_deg)))
    if not valid:
        raise ValueError("no finite event-state phase samples")
    # Search all samples for the narrowest sign-changing bracket.
    pairs = []
    sv = sorted(valid)
    for (z0, e0), (z1, e1) in zip(sv[:-1], sv[1:]):
        if e0 == 0.0:
            return float(np.clip(z0, -z_bound, z_bound))
        if e1 == 0.0:
            return float(np.clip(z1, -z_bound, z_bound))
        if e0 * e1 < 0.0:
            pairs.append((abs(z1-z0), z0, e0, z1, e1))
    if pairs:
        _, z0, e0, z1, e1 = min(pairs)
        znew = z1 - e1*(z1-z0)/(e1-e0)
        if not (min(z0,z1) < znew < max(z0,z1)):
            znew = 0.5*(z0+z1)
        return float(np.clip(znew, -z_bound, z_bound))
    # Empirical secant when two distinct event-state samples are available.
    if len(valid) >= 2:
        z0, e0 = valid[-2]
        z1, e1 = valid[-1]
        dz = z1-z0
        de = e1-e0
        if abs(dz) > 1e-10 and abs(de/dz) > 1e-3:
            step = float(np.clip(-e1/(de/dz), -abs(max_step), abs(max_step)))
            return float(np.clip(z1+step, -z_bound, z_bound))
    # First sample: use the exact backend one-step matrix locally.
    z1, e1 = valid[-1]
    h = 1e-3
    pp = phase_from_response_z(response_matrix, z1+h)
    pm = phase_from_response_z(response_matrix, z1-h)
    deriv = angle_error_deg(pp, pm)/(2*h)
    if not np.isfinite(deriv) or abs(deriv) < 1e-3:
        step = -math.copysign(min(abs(max_step), 0.75), e1)
    else:
        step = float(np.clip(-e1/deriv, -abs(max_step), abs(max_step)))
    return float(np.clip(z1+step, -z_bound, z_bound))
...
def _j_wrapper_factory(original_compute, context):
    def wrapped(mesh, u, sigma_gp, psi_e_gp, d, crack_tip, crack_direction,
        ...
        reliable = bool(probe.get("reliable", False))
        if reliable:
...
def _summary(out, T, context, base_summary):
    ...
    payload = {
        ...
        "loading_z": loading_z_from_coefficients(context.loading_open_coeff, context.loading_shear_coeff),
        "target_traction_phase_deg": context.target_traction_phase_deg,
...
def main(argv=None):
    ...
    if args.mode != "2d":
        raise SystemExit("v6 anisotropic mixed mode requires --mode 2d")
    if not bool(getattr(args, "crystal_aniso", False)):
        raise SystemExit("v6 requires --crystal-aniso")
    if not bool(getattr(args, "crystal_compete", False)):
        raise SystemExit("v6 requires --crystal-compete")
    if bool(getattr(args, "crystal_branch", False)) or int(getattr(args, "max_fronts", 1)) != 1:
        raise SystemExit("v6 first-passage screen requires branching off and --max-fronts 1")
    ...
    print("MIXED_MODE_V6 complete:", json.dumps(vals, indent=2, default=str))
    return vals
