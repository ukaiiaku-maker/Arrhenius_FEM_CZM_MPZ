"""Event-controlled mixed-mode first-passage wrapper for the active FEM/CZM sharp-front solver.
...
MODEL_ID = "FEM_CZM_mixed_mode_first_passage_v2_event_controlled_multiring"
...
def _j_wrapper_factory(original_compute, context: MixedModeContext):
    def wrapped(mesh, u, sigma_gp, psi_e_gp, d, crack_tip, crack_direction,
        ...
        md = project_near_tip_modes(mesh, sigma_gp, d, crack_tip, crack_direction,
                                    rmin, rmax, context.projection_angle_deg, context.projection_damage_cutoff)
...
def _accepted_first_passage(out: Path, T: float, context: MixedModeContext, summary: list[dict]) -> dict:
    ...
    payload = {
        ...
        "projection_method": "full_field_multiring_isotropic_Williams_IRLS",
        "projection_n": mode.get("mode_projection_n"),
        "projection_fit_count": mode.get("mode_projection_fit_count"),
        "projection_rel_rmse": mode.get("mode_projection_rel_rmse"),
        "projection_condition": mode.get("mode_projection_condition"),
        "projection_psi_spread_deg": mode.get("mode_projection_psi_spread_deg"),
        "projection_K_spread_frac": mode.get("mode_projection_K_spread_frac"),
        "projection_reliable": mode.get("mode_projection_reliable"),
        "projection_rmin_m": mode.get("mode_projection_rmin_m"),
        "projection_rmax_m": mode.get("mode_projection_rmax_m"),
        "projection_angle_deg": mode.get("mode_projection_angle_deg"),
...
def main(argv=None):
    ...
    if args.mode != "2d":
        raise SystemExit("mixed_mode_first_passage_v2 requires --mode 2d")
    if int(getattr(args, "max_fronts", 1)) != 1 or bool(getattr(args, "crystal_branch", False)):
        raise SystemExit("v2 mixed-mode first-passage requires branching off and --max-fronts 1")
    ...
    (out/"mixed_mode_run_config.json").write_text(json.dumps({
        ...
        "note": "deterministic H=1 by default; loading angle is iteratively controlled by the v2 campaign at the first-passage or censoring state",
    }, indent=2))
    ...
    print("MIXED_MODE_V2 complete:", json.dumps(payloads, indent=2, default=str))
    return payloads
