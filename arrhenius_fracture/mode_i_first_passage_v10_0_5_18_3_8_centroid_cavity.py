"""v10.0.5.18.3.8 atomic centroid-cavity refinement entry."""
...
from .quality_aware_czm_centroid_cavity_v10051838 import (
    MODEL_ID as CENTROID_MODEL,
    install as install_centroid_cavity,
)
...
MODEL_ID = (
    ...
    "centroid_steiner_cavity_v10_0_5_18_3_8"
)
...
def _fields() -> dict[str, Any]:
    return {
        ...
        "model": MODEL_ID,
        "centroid_steiner_cavity_refinement_active": True,
        "centroid_steiner_cavity_model": CENTROID_MODEL,
        "centroid_split_child_count": 3,
        "centroid_split_nominal_child_parent_area_ratio": 1.0 / 3.0,
        "centroid_split_final_quality_gate": True,
        "centroid_split_intermediate_topology_exposed": False,
        "centroid_split_neighbour_elements_modified": False,
        "centroid_split_gauss_parent_map_composed": True,
        "centroid_split_nodal_state_interpolated": True,
...
def _annotate(payload: dict[str, Any], *, name: str, error: BaseException | None) -> dict[str, Any]:
    fields = _fields()
    ...
    if name == RETRY_AUDIT:
        payload["centroid_cavity_fallback_after_single_edge_exhaustion"] = True
    return payload
...
def _rewrite_outputs(out: Path | None, error: BaseException | None) -> None:
    ...
    for old_name, new_name in _RENAMES.items():
        ...
        if not old_path.is_file():
            continue
        payload = _annotate(
            dict(json.loads(old_path.read_text()) or {}),
            name=new_name,
            error=error,
        )
    ...
    if corridor_path.is_file():
        payload = _annotate(
            dict(json.loads(corridor_path.read_text()) or {}),
            name=corridor_path.name,
            error=error,
        )
...
def main(argv: list[str] | None = None):
    ...
    _local37.install_ray_handoff = install_centroid_cavity
