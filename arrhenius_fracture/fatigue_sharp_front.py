...
def _build_front(args, mat: ElasticProperties) -> FrontEngine:
    ...
    f.dN_cap = args.dN_cap
    f.beta_back = args.beta_back
    f.c_blunt = args.c_blunt
    f.L_pz = args.L_pz
    f.v_emb_b3 = args.v_emb_b3
    f.wake_retain = args.wake_retain
    f.chi_shield = args.cleave_shield_chi
    f.emb_sat_frac = args.emb_sat_frac
    f.N_sat = args.N_sat
    f.recover_k = args.recover_k
    f.da = args.da
    f.rho0 = args.rho0

    cb = apply_cleavage_barrier_args(default_cleavage_barrier(), args)

    # Legacy fatigue uses a separate controller surface.  In moving-PZ mode the
    # same front emission/transport parameters are used for monotonic, cyclic
    # and dwell loading, so the controller only supplies waveform integration
    # and adaptive block sizing.
    eb_compat = apply_emission_barrier_args(default_emission_barrier(mat.b), args)
    state_model = str(getattr(args, 'front_state_model', 'legacy_scalar') or 'legacy_scalar')
    if state_model == 'moving_pz':
        ...
        f.sigma_cap = f.sigma_cap if bool(getattr(args, 'mpz_allow_sigma_cap', False)) else 0.0
        f.dN_cap = float('inf'); f.N_sat = float('inf'); f.chi_shield = 0.0
        f.v_emb_b3 = 0.0; f.beta_back = 0.0; f.wake_retain = 1.0
        mpz_cfg = build_mpz_config_from_namespace(args, default_length_m=max(f.L_pz, 1e-12))
        return MovingProcessZoneFrontEngine(f, cb, eb_compat, mat.G, mat.nu, mat.b, mpz_cfg)
    return FrontEngine(f, cb, eb_compat, mat.G, mat.nu, mat.b)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    # The moving-PZ fatigue path uses this same emission surface as monotonic
    # fracture. Legacy fatigue ignores these in favor of its historical scaled
    # controller barriers.
    p.add_argument('--emit-barrier-kind', choices=['classic','exp_floor'], default=None, dest='emit_barrier_kind')
