"""Exact PF v10.4.1 full-field Peierls--Taylor detailed-balance rates.

This module is an isolated port of the bulk kinetic closure used by
PF-fracture-fatigue branch ``v10.4.1-bulk-detailed-balance``.  It is intentionally
separate from the moving-tip MPZ kinetics.  No tip population is deposited into
the bulk; the two populations interact only through the common FEM stress and J.
"""
...
MODEL_ID = "v10.4.1_bulk_peierls_taylor_detailed_balance_exact_port"
...
class ExpFloorSurface:
    ...
    def barrier_eV(self, stress_Pa: np.ndarray, T_K: float) -> np.ndarray:
        ...
        return np.maximum(
            ...
            * np.exp(
                -max(self.alpha, 0.0)
                * np.power(x, max(self.exponent, 1.0e-12))
            ),
            0.0,
        )

    def one_way_rate_s(self, stress_Pa: np.ndarray, T_K: float) -> np.ndarray:
    ...
    def rate_s(self, stress_Pa: np.ndarray, T_K: float) -> np.ndarray:
        """PF v10.4.1 net rate, Gamma_forward minus Gamma_reverse."""
        stress = np.maximum(np.asarray(stress_Pa, dtype=float), 0.0)
        kT = max(KB_EV_PER_K * float(T_K), 1.0e-30)
        forward_barrier = self.barrier_eV(stress, T_K)
        zero_barrier = self.barrier_eV(np.zeros_like(stress), T_K)
        reverse_barrier = np.maximum(
            2.0 * zero_barrier - forward_barrier,
            zero_barrier,
        )
        forward = self.attempt_frequency_s * np.exp(
            np.clip(-forward_barrier / kT, -745.0, 0.0)
        )
        reverse = self.attempt_frequency_s * np.exp(
            np.clip(-reverse_barrier / kT, -745.0, 0.0)
        )
...
class EmissionDerivedPeierlsTaylorModel:
    ...
    def rates(
        ...
        peierls_barrier = self.cfg.peierls.barrier_eV(stress, T_K)
        taylor_barrier = self.cfg.taylor.barrier_eV(taylor_local_stress, T_K)
        return {
            ...
            "rho_mobile_m2": mobile_density,
            "jump_length_m": jump_length,
            "peierls_barrier_eV": peierls_barrier,
            "G_peierls_eV": peierls_barrier,
            "taylor_barrier_eV": taylor_barrier,
            "G_taylor_eV": taylor_barrier,
