...
def build_engine(args, mat) -> FrontEngine:
    ...
    f.max_advances_per_step = getattr(args, 'max_advances_per_step', float('inf'))
    f.dN_cap = getattr(args, 'dN_cap', float('inf'))
    f.da = args.da
    if getattr(args, 'rho0', None) is not None:
        f.rho0 = float(args.rho0)

    state_model = str(getattr(args, 'front_state_model', 'legacy_scalar') or 'legacy_scalar').lower()
    f.state_model = state_model
    # v9 moving-PZ runs disable the old cohesive stress ceiling by default.
    # It remains available as an explicit diagnostic option for regression runs.
    if state_model == 'moving_pz' and not bool(getattr(args, 'mpz_allow_sigma_cap', False)):
        f.sigma_cap = 0.0
    # These legacy scalar couplings are not constitutive inputs to the moving-PZ
    # engine. Retain their CLI values only so old commands remain reproducible.
    if state_model == 'moving_pz':
        f.dN_cap = float('inf')
        f.N_sat = float('inf')
        f.chi_shield = 0.0
        f.v_emb_b3 = 0.0
    ...
    eb = apply_emission_barrier_args(default_emission_barrier(mat.b), args)
    # The active moving-PZ production model uses direct EXP-floor surfaces for
    # both bond opening and crack-tip emission. Classic H-TS-sigma*v barriers
    # remain available only when explicitly requested for a legacy ablation.
    if state_model == 'moving_pz':
        if getattr(args, 'cleave_barrier_kind', None) is None:
            cb.barrier_kind = 'exp_floor'
        if getattr(args, 'emit_barrier_kind', None) is None:
            eb.barrier_kind = 'exp_floor'
    if args.emit_H0_eV is not None: eb.H0_eV = args.emit_H0_eV
    if getattr(args, 'emit_S0_kB', None) is not None:
        eb.S0_neg_kB = args.emit_S0_kB
    if getattr(args, 'emit_sigma0_S', None) is not None:
        eb.sigma0_S_GPa = args.emit_sigma0_S
    if getattr(args, 'entropy_form', None) is not None:
        cb.entropy_stress_form = args.entropy_form
        eb.entropy_stress_form = args.entropy_form
        # affine on emission would make its zero-stress barrier collapse with T;
        # the user asked for it explicitly, so honor it but it is mainly a
        # diagnostic / sweep lever (see fatigue-limit discussion).
        cb.use_negative_entropy = True
        eb.use_negative_entropy = True
    if getattr(args, 'entropy_gate_power', None) is not None:
        cb.entropy_gate_power = args.entropy_gate_power
        eb.entropy_gate_power = args.entropy_gate_power
    # physical composite-entropy overrides (emission channel; the dominant one)
    if getattr(args, 'emit_S_T_c0_kB', None) is not None:
        eb.S_T_c0_kB = args.emit_S_T_c0_kB
    if getattr(args, 'emit_S_T_c1', None) is not None:
        eb.S_T_c1_kB_per_K = args.emit_S_T_c1
    if getattr(args, 'emit_S_sigma_max_kB', None) is not None:
        eb.S_sigma_max_kB = args.emit_S_sigma_max_kB
    if getattr(args, 'cleave_S_sigma_max_kB', None) is not None:
    ...
    if state_model == 'moving_pz':
        ...
        mpz_cfg = build_mpz_config_from_namespace(args, default_length_m=max(float(f.L_pz), 1e-12))
        return MovingProcessZoneFrontEngine(f, cb, eb, mat.G, mat.nu, mat.b, mpz_cfg)
...
def run_2d(args):
    ...
    for T in args.temperatures:
        ...
        # NOTE: this setup block is deliberately placed BEFORE
        # `prev_a_tip_for_block = float(a_tip)` (rather than after, which
        # would be the more natural reading order) so that the exact
        # original adjacency between that line and `while step <
        # args.steps:` is preserved byte-for-byte -- several other modules
        # (see mode_i_first_passage_v10_0_5_5_stochastic_vhcf.py's
        # _CACHE_INIT_ANCHOR) locate an AST/text-patch insertion point by
        # searching for that exact literal text.
        ...
        while step < args.steps:
            ...
            while True:
                ...
                if pf_target_active:
                    ...
                    # (FEM_CZM_HANDOFF.md section 6): determines the Uapp
                    # that reproduces the PF reference KJ at this trial's
                    # physical time via a safeguarded predictor+secant
                    # search. `_pf_solve_trial` below is PURELY LOCAL -- it
                    # never touches the shared u/ep_gp/rho_gp/sigma_gp/
                    # psi_gp/Ftop arrays, only its own local copies -- so
                    # the only effect of this whole block is choosing
                    # `dU_step` a few lines down. The unconditional
                    # mechanics block that actually updates the shared
                    # state afterward is therefore never itself made
                    # conditional or re-indented: several other modules
                    # AST/text-patch that exact block by literal match
                    # (see the "sharp_front.py architecture" notes in
                    # CLAUDE_PROGRESS.md).
                    ...
                    def _pf_solve_trial(_pf_Uapp_trial):
                        # Captured BEFORE this candidate's local arrays are
                        # (re)initialized from u_saved/ep_saved/rho_saved:
                        # this is simultaneously "before this candidate's
                        # solve" and "after restoring the previous
                        # candidate" (the two are the same instant -- nothing
                        # shared was ever mutated by the previous candidate
                        # to begin with).
                        _pf_fp_candidates.append(_pf_capture_fingerprint(eng))
                        _pf_u_local = u_saved.copy()
                        _pf_ep_local = ep_saved.copy()
                        _pf_rho_local = rho_saved.copy()
                        _pf_Uy_top = 0.5 * _pf_Uapp_trial
                        _pf_Uy_bot = -0.5 * _pf_Uapp_trial
                        _pf_sigma_local = np.zeros((3, mesh.ne))
                        _pf_psi_local = np.zeros(mesh.ne)
                        for _pf_it in range(args.n_stagger):
                            (_pf_Kmat, _pf_Rint, _pf_sigma_local, _pf_seq_local,
                             _pf_s1_local, _pf_psi_local) = assemble_mechanics(
                                mesh, _pf_u_local, _pf_ep_local, _pf_rho_local, d, D, mat,
                                cohesive_network=cohesive_network)
                            _pf_u_local, _pf_Ftop_local = solve_dirichlet(
                                _pf_Kmat, _pf_Rint, _pf_u_local, bnd, _pf_Uy_top, _pf_Uy_bot)
                            (_pf_Kmat, _pf_Rint, _pf_sigma_local, _pf_seq_local,
                             _pf_s1_local, _pf_psi_local) = assemble_mechanics(
                                mesh, _pf_u_local, _pf_ep_local, _pf_rho_local, d, D, mat,
                                cohesive_network=cohesive_network)
                            _pf_ep_local, _pf_rho_local, _pf_dot_ep_local = update_plasticity(
                                _pf_ep_local, _pf_rho_local, _pf_sigma_local, mat, T, dt_cur,
                                plast_model, cfg.dislocations)
                        _pf_h_local = mesh.hbar_tip if mesh.hbar_tip > 0 else mesh.hbar
                        _, _pf_KJ_trial, _ = compute_J_integral(
                            mesh, _pf_u_local, _pf_sigma_local, _pf_psi_local, d,
                            np.array([a_tip, 0.0]), np.array([1.0, 0.0]), mat,
                    ...
                    # accepted-step hazard commit further below -- a
                    # genuine transactionality violation, not something to
                    ...
                    dU_step = _pf_result.accepted_Uapp - Uapp_saved
                elif fatigue_mode and bool(getattr(args, 'fatigue_hold_load', False)) and step > 0:
                    dU_step = 0.0
                else:
