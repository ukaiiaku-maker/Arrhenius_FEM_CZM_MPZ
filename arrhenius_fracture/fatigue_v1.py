...
class FatigueCycleHazardController:
    ...
    def integrate_one_cycle(self, front, waveform: FatigueWaveform, T_K: float) -> CycleHazardResult:
        if (getattr(front, 'state_model', 'legacy_scalar') == 'moving_pz'
                and hasattr(front, 'predict_fatigue_cycle')):
            pz = front.predict_fatigue_cycle(waveform, T_K, self.cfg.n_phase)
            # The moving-PZ engine uses its own shared emission/transport barriers.
            # These fields feed the existing adaptive cycle-block controller only.
            return CycleHazardResult(
                mu_emit=float(pz['dN_emit_per_cycle']),
                mu_peierls=0.0,
                mu_taylor=0.0,
                mu_escape=0.0,
                mu_cleave=float(pz['mu_cleave_per_cycle']),
                store_per_cycle=float(pz['dN_emit_per_cycle']),
                mobile_per_cycle=float(pz['dN_emit_per_cycle']),
                escape_per_cycle=0.0,
                peierls_per_cycle=0.0,
                taylor_per_cycle=0.0,
                avg_sigma_tip=float(pz['avg_sigma_tip']),
                max_sigma_tip=float(pz['max_sigma_tip']),
                avg_sigma_emit_eff=float(pz['avg_sigma_emit_eff']),
    ...
    def cycle_step_front(
        ...
        if (getattr(front, 'state_model', 'legacy_scalar') == 'moving_pz'
                and hasattr(front, 'commit_fatigue_block')):
            out = front.commit_fatigue_block(waveform, T_K, cycles, self.cfg.n_phase)
            out['cycle_limiter'] = str(stepdiag.get('limiter', 'unknown'))
            out['cycle_unlimited'] = float(stepdiag.get('unlimited_cycles', cycles))
            out['cycle_candidate_limits'] = json.dumps(stepdiag.get('candidate_limits', {}), sort_keys=True)
            return out
