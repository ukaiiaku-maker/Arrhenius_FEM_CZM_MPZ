...
OUT=Path("analysis_outputs/oneD_v2_capability_v2");OUT.mkdir(parents=True,exist_ok=True)
PF="ab6279331d050919f78e2e7f278bc332466e34f8";FEM="931bed66913afc970117bce900805ecc9b6225f8"
def sha(p):return hashlib.sha256(p.read_bytes()).hexdigest()
pairs={}
physical=[]
for cls in ("peak","dbtt"):
 a=Path(f"/private/tmp/oneD_v2_pf_{cls}_control");b=Path(f"/private/tmp/oneD_v2_pf_{cls}_instrumented")
 names=sorted(set(x.name for x in a.iterdir())&set(x.name for x in b.iterdir()))
 authoritative=[n for n in names if n in {"steps_1000K.csv","fronts_1000K.csv","branch_diagnostics_1000K.csv","crack_path_1000K.csv","crack_path_front0_1000K.csv","sharp_wake_advance_log.csv","stochastic_avalanche_geometry_events.json","summary.json","v10_2_30_hazard_energy_gate_audit.json","kinetic_tip_cell_audit_v101.json"}]
 pairs[cls]={n:sha(a/n)==sha(b/n) for n in authoritative};events=json.loads((b/"stochastic_avalanche_geometry_events.json").read_text())
 physical.append({"material_class":cls,"event_count":len(events),"all_authoritative_byte_identical":all(pairs[cls].values()),"thresholds":[e["threshold_action"] for e in events],"event_lengths_m":[e["event_advance_m"] for e in events],"geometry_commit_counts":[e["realized_geometry_commits"] for e in events],"endpoint_errors_m":[e["event_length_error_m"] for e in events]})
...
(OUT/"oneD_v2_rng_threshold_audit.json").write_text(json.dumps(rng,indent=2)+"\n")
manifest={"PF":{"source_commit":PF,"bulk_constraint":"PLANE_STRAIN","crack_representation":"FINITE_WIDTH_STIFFNESS_KILLED_WAKE","failed_region_state":"BINARY_STIFFNESS_KILL","local_process_zone":"FINITE_SIZE_REDUCED_STATE","continuum_G_status":"UNQUALIFIED","map_status":"NOT_EVALUATED"},"FEMCZM":{"source_commit":FEM,"bulk_constraint":"PLANE_STRAIN","crack_representation":"CODIMENSION_ONE_DISPLACEMENT_DISCONTINUITY","failed_interface_state":"TRACTION_FREE","local_process_zone":"FINITE_SIZE_REDUCED_STATE","native_J_map_status":"NOT_EVALUATED","qualified_G_map_status":"NOT_EVALUATED"},"interpolation":"BOUNDED_PIECEWISE_LINEAR_NO_EXTRAPOLATION","campaign_outputs_populated":False}
