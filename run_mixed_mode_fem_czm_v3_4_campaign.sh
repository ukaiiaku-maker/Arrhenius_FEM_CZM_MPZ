...
OUTROOT="${OUTROOT:-runs/mixed_mode_fem_czm_v3_4_campaign_500K}"
...
# Reuse a supplied v3.3 calibration when present. Set RECALIBRATE=1 to
# force regeneration with the validated circular-phase calibrator.
CALCSV="$CALROOT/mixed_mode_loading_calibration_v3_3.csv"
...
conda run -n "$CONDA_ENV" python run_mixed_mode_fem_czm_v3_4_campaign.py \
...
conda run -n "$CONDA_ENV" python plot_mixed_mode_fem_czm_v3_4_results.py --root "$OUTROOT"
