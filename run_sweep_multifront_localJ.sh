#!/usr/bin/env bash
set -euo pipefail
OUT=${1:-sweep_multifront_localJ}
mkdir -p "$OUT"
COMMON="--mode 2d --nx 50 --ny 100 \
  --tip-h-fine 0.35e-6 --tip-ratio 1.22 \
  ...
  --adaptive-grow 4.0 --max-fronts 32 --branch-spacing 10 \
  --rJ-outer 4e-6 --branch-resolve-length 4e-6"
