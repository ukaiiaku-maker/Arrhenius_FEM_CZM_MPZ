...
T_K=${T_K:-700}
OUTROOT=${OUTROOT:-runs/v10_stageC_progressive_${MATERIAL}_${T_K}K_5um_v1}
STEPS=${STEPS:-50000}
NX=${NX:-36}
NY=${NY:-72}
...
"$PYTHON_BIN" -m arrhenius_fracture.mode_i_first_passage_v10_0_progressive \
...
"$PYTHON_BIN" audit_v10_0_progressive_smoke.py \
  "$OUTROOT" --target-um 5

cat <<EOF
PROGRESSIVE ONE-SEGMENT GATE PASSED
material=$MATERIAL
T_K=$T_K
out=$OUTROOT
Do not launch a longer run until the output archive has been inspected and penalty convergence has passed.
EOF
