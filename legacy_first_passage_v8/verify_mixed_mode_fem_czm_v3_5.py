...
def main():
    ...
    print('mechanics model:',MODEL_ID)
    print('calibration: mixed_mode_fem_czm_v3_5_matrix_authoritative')
    ...
    suite.addTests(unittest.defaultTestLoader.loadTestsFromName('tests.test_mixed_mode_calibration_v3_5'))
    suite.addTests(unittest.defaultTestLoader.loadTestsFromName('tests.test_mixed_mode_campaign_v3_5'))
    result=unittest.TextTestRunner(verbosity=2).run(suite)
    if not result.wasSuccessful():raise SystemExit(1)
    print('MIXED_MODE_V3_5 verification OK')
