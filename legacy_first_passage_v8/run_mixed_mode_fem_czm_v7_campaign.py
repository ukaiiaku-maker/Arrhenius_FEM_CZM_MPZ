#!/usr/bin/env python3
"""Deterministic anisotropic mixed-mode campaign with exact backend full-circle control, v7."""
...
RUNNER_ID = "mixed_mode_fem_czm_v7_exact_backend_full_circle_event_control"
REQ = {
...
def main():
    ...
    p.add_argument("--outroot", default="runs/mixed_mode_fem_czm_v7_production_backend_500K")
    ...
    probe = subprocess.run(
        [py, "-c", "from arrhenius_fracture.mixed_mode_first_passage_v7 import MODEL_ID;print(MODEL_ID)"],
        capture_output=True, text=True)
    if probe.returncode:
    ...
    def run_one(class_name, target):
        ...
        pd.DataFrame(history).to_csv(root / "mixed_mode_control_history_v7.csv", index=False)
        if not candidates:
    ...
    pd.DataFrame(results).to_csv(out / "campaign_status_v7.csv", index=False)
    good = [x for x in results if x.get("status") != "failed"]
    if good:
        pd.DataFrame(good).to_csv(out / "mixed_mode_v7_anisotropic_all_cases.csv", index=False)
    if any(x.get("status") == "failed" for x in results):
        raise SystemExit("one or more v7 cases failed")
