#!/usr/bin/env python3
"""Deterministic anisotropic mixed-mode campaign with exact backend control, v6."""
...
RUNNER_ID = "mixed_mode_fem_czm_v6_production_backend_event_control"
REQ = {
...
def main():
    ...
    p.add_argument("--outroot", default="runs/mixed_mode_fem_czm_v6_production_backend_500K")
    ...
    p.add_argument("--max-z-step", type=float, default=1.5)
    p.add_argument("--z-bound", type=float, default=14.0)
    a = p.parse_args()

    py = pyenv(a.conda_env)
    probe = subprocess.run(
        [py, "-c", "from arrhenius_fracture.mixed_mode_first_passage_v6 import MODEL_ID;print(MODEL_ID)"],
        capture_output=True, text=True)
    if probe.returncode:
    ...
    def run_one(class_name, target):
        ...
        for iteration in range(max(1, a.max_control_iters)):
            ...
            if rc:
                history.append({"iteration": iteration, "loading_z": z,
            ...
            reliable = bool(zsum.get("traction_probe_reliable", False))
            rowhist = {
                "iteration": iteration,
                "loading_z": z,
            ...
            if not np.isfinite(achieved):
                break
            znew = safeguarded_event_z_update(
                history, target, M, max_step=a.max_z_step, z_bound=a.z_bound)
            if abs(znew - z) < 1e-8:
                break
            z = znew

        pd.DataFrame(history).to_csv(root / "mixed_mode_control_history_v6.csv", index=False)
        if not candidates:
        ...
        def rank(item):
            ...
            ok = bool(h["traction_probe_reliable"] and e <= a.event_psi_tol_deg)
        ...
        final = {
            **zsum, **audit,
            ...
            "selected_loading_z": float(h["loading_z"]),
            ...
            "calibration_loading_z": float(cal["loading_z"]),
    ...
    with ThreadPoolExecutor(max_workers=max(1, a.max_jobs)) as ex:
        ...
        for future in as_completed(futures):
            ...
            print({q: z.get(q) for q in (
                "class", "target_psi_deg", "status", "selected_loading_z",
    ...
    pd.DataFrame(results).to_csv(out / "campaign_status_v6.csv", index=False)
    good = [x for x in results if x.get("status") != "failed"]
    if good:
        pd.DataFrame(good).to_csv(out / "mixed_mode_v6_anisotropic_all_cases.csv", index=False)
    if any(x.get("status") == "failed" for x in results):
        raise SystemExit("one or more v6 cases failed")
