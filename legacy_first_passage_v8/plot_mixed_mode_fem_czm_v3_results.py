...
def main():
    ...
    f=a.root/'mixed_mode_v3_all_cases.csv'
    if not f.exists():raise SystemExit(f'missing {f}')
    d=pd.read_csv(f);out=a.root/'plots_v3';out.mkdir(parents=True,exist_ok=True)
    ...
    ax.axhline(0,lw=.8);ax.axvline(0,lw=.8);ax.set_xlabel(r'$K_I$ [MPa$\sqrt{m}$]');ax.set_ylabel(r'$K_{II}$ [MPa$\sqrt{m}$]');ax.set_title('J-consistent isotropic mixed-mode response');ax.grid(alpha=.25)
    ...
    d.to_csv(out/'mixed_mode_v3_plot_data.csv',index=False)
    print('wrote',out)
