#!/usr/bin/env python3
"""Censor-aware plots for mixed-mode FEM/CZM v8."""
...
def main():
    ...
    a = p.parse_args()
    out = a.out or (a.root/"plots_v8")
    out.mkdir(parents=True, exist_ok=True)
    csv_path = a.root/"mixed_mode_v8_anisotropic_all_cases.csv"
    if not csv_path.exists():
        raise SystemExit(f"missing {csv_path}")
    df = pd.read_csv(csv_path)
    df["event_observed"] = df["status"].eq("event") | df["status"].eq("event_phase_mismatch")
    df.to_csv(out/"mixed_mode_v8_plot_data.csv", index=False)
    ...
    ax.set_title("Production-backend controlled anisotropic first passage")
    ax.grid(alpha=.25)
