#!/usr/bin/env python3
"""Two-basis isotropic elastic calibration for mixed-mode FEM/CZM v3.1.

The authoritative amplitude in v3.1 comes from the domain J integral.  This
...
MODEL_ID = "FEM_CZM_mixed_mode_first_passage_v3_1_J_consistent_phase_ratio"
...
def main():
    ...
    p.add_argument('--out',default='runs/mixed_mode_fem_czm_v3_1_phase_calibration')
    ...
    def evaluate(alpha):
        ...
        usable,reasons=phase_projection_gate(row,max_condition=a.projection_condition_max,
                                              max_phase_spread_deg=a.phase_spread_tol_deg)
        row['phase_projection_usable']=usable
        row['phase_projection_reasons']=';'.join(reasons)
    ...
    M=np.array([[opening['KI_MPa_sqrt_m'],sliding['KI_MPa_sqrt_m']],
                [opening['KII_MPa_sqrt_m'],sliding['KII_MPa_sqrt_m']]],float)
    ...
    grid=[evaluate(x) for x in vals(a.diagnostic_alpha_grid_deg)]
    mappings=[]
    for target in vals(a.target_psi_deg):
        try:
            alpha=loading_angle_from_mode_basis(M,target,max_abs_alpha_deg=85.0)
            ...
            mappings.append({'target_psi_deg':target,'loading_angle_deg':np.nan,
                             ...
                             'calibration_model':'two_basis_phase_ratio'})
            continue
        hist=[]
        for it in range(max(1,a.max_refine_iters)):
            r=evaluate(alpha);err=angle_error_deg(r['achieved_psi_deg'],target)
            ...
            rp=evaluate(min(alpha+da,89.0));rm=evaluate(max(alpha-da,-89.0))
            ...
            if not np.isfinite(slope) or abs(slope)<0.05:
                break
            alpha=float(np.clip(alpha-err/slope,-85.0,85.0))
    ...
    datasets=[('elastic_alpha_grid_v3_1.csv',grid),('mixed_mode_loading_calibration_v3_1.csv',mappings)]
    ...
    payload={'model':MODEL_ID,'settings':vars(a),'basis_matrix':M.tolist(),
             'basis_condition':basis_cond,'opening_basis':opening,'sliding_basis':sliding,
             'mapping':mappings}
    (out/'mixed_mode_loading_calibration_v3_1.json').write_text(json.dumps(payload,indent=2))

    print('mode basis matrix [MPa sqrt(m) per calibration amplitude]:')
    print(M)
    ...
    print('wrote',out/'mixed_mode_loading_calibration_v3_1.csv')
