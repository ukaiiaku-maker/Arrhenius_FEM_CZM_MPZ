...
from shared_entropy_family_v2 import build_parent_barrier, multihit_rate
...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument("--out", type=Path, default=Path("runs/panels_CD_entropy_family_v2"))

    # Shared entropy family.
    p.add_argument("--A-T-kB", type=parse_float_list,
                   default=parse_float_list("0 4 8 12 16"),
                   help="temperature-entropy magnitudes; Panel D axis and Panel C color")
...
def run_sweeps(args, outdir: Path) -> tuple[pd.DataFrame, pd.DataFrame]:
    cpath = outdir / "panelC_entropy_SN_raw_v2.csv"
    dpath = outdir / "panelD_entropy_strength_raw_v2.csv"
...
def plot_D(args, d: pd.DataFrame, outdir: Path) -> None:
    ...
    norm = norm_for(good, "Lambda_S_ref")
    cmap = plt.get_cmap("viridis")
    fig = plt.figure(figsize=(10.5, 8.2))
    ax = fig.add_subplot(111, projection="3d")
    ...
    for (A_T, A_sigma, sigmaS, lam), g in good.groupby(group_cols, sort=True):
        ...
        ax.plot(g.T_K, np.full(len(g), float(A_T)), g.sigma_y_MPa,
                color=color, lw=1.65, alpha=0.92)
        ax.scatter(g.T_K, np.full(len(g), float(A_T)), g.sigma_y_MPa,
                   color=[color], s=11)

    ax.set_xlabel("Temperature [K]", labelpad=10)
    ax.set_ylabel(r"Temperature-entropy magnitude $A_T$ [$k_B$]", labelpad=10)
    ax.set_zlabel(r"Yield strength $\sigma_y$ [MPa]", labelpad=11)
    ax.view_init(elev=args.view_elev, azim=args.view_azim)
    sm = ScalarMappable(norm=norm, cmap=cmap); sm.set_array([])
    cb = fig.colorbar(sm, ax=ax, fraction=0.045, pad=0.10)
    cb.set_label(r"Entropic barrier ratio $\Lambda_{S,\mathrm{ref}}$")
    fig.tight_layout(rect=(0,0.03,0.94,1))
    fig.savefig(outdir/"panelD_entropy_strength_AT_waterfall_3d.png", dpi=args.figure_dpi, bbox_inches="tight")
    fig.savefig(outdir/"panelD_entropy_strength_AT_waterfall_3d.pdf", bbox_inches="tight")
    plt.close(fig)

    # 2-D diagnostic, colored by Lambda_S_ref.
    ...
    for ax2, AT in zip(axes[0], ATs):
        ...
        for lam, g in q.groupby("Lambda_S_ref", sort=True):
            g = g.sort_values("T_K")
            ax2.plot(g.T_K, g.sigma_y_MPa, lw=1.4, color=cmap(norm(float(lam))))
        ax2.set_title(rf"$A_T={AT:g}$")
        ax2.set_xlabel("T [K]")
        ax2.set_ylabel(r"$\sigma_y$ [MPa]")
        ax2.grid(alpha=0.25)
    fig.tight_layout()
    fig.savefig(outdir/"panelD_strength_temperature_diagnostic_v2.png", dpi=args.figure_dpi, bbox_inches="tight")
    plt.close(fig)
...
def curve_summaries(args, c: pd.DataFrame, d: pd.DataFrame, outdir: Path) -> None:
    ...
    pd.DataFrame(crows).to_csv(outdir/"panelC_curve_summary_v2.csv", index=False)
    ...
    pd.DataFrame(drows).to_csv(outdir/"panelD_curve_summary_v2.csv", index=False)
...
def write_metadata(args, outdir: Path) -> None:
    ...
    pd.DataFrame(rows).to_csv(outdir/"panels_CD_entropy_family_design_v2.csv", index=False)
    manifest = {
        ...
        "panel_D_axes": {"x":"T_K","y":"A_T_kB","z":"sigma_y_MPa","color":"Lambda_S_ref"},
    ...
    (outdir/"panels_CD_manifest_v2.json").write_text(json.dumps(manifest, indent=2))
