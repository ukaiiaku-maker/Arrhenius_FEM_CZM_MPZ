#!/usr/bin/env python3
"""Summarize sharp-front 2-D branch sweeps.

Scans a sweep root for summary.json files and branch_diagnostics_*.csv files.
Writes a flat CSV with initiation metrics and simple morphology labels.
"""
...
def _extract_from_args(d: dict, key: str, default=np.nan):
    try:
...
def summarize_case(case_dir: Path):
    ...
    try:
        ...
        print(f'WARN could not read {sfile}: {e}')
        return []
    try:
    ...
    for s in summaries:
        ...
        if diag is None:
            # tolerate old nonzero-padded naming
            candidates = list(case_dir.glob(f'branch_diagnostics_*{int(round(T))}K.csv'))
            diag = _safe_read_csv(candidates[0]) if candidates else None
        dmax = {}
        if diag is not None and len(diag):
            # prefer the first spawned row, else the row with max O2/O1
            if 'branch_spawned' in diag.columns and diag['branch_spawned'].astype(bool).any():
                ...
                r = diag.iloc[int(np.nanargmax(diag['metric2_over_metric1'].to_numpy(float)))]
            else:
                r = diag.iloc[-1]
        ...
        row = dict(
            case=str(case_dir),
            T_K=T,
            theta_deg=_extract_from_args(args, 'crystal_theta_deg'),
            gamma_aniso=_extract_from_args(args, 'cleave_gamma_aniso'),
            branch_overdrive_ratio=_extract_from_args(args, 'branch_overdrive_ratio'),
            branch_ratio=_extract_from_args(args, 'branch_ratio'),
            material=_extract_from_args(args, 'crystal_material_resolved', _extract_from_args(args, 'crystal_material', '')),
            ...
            length1_m=p1['length_m'], angle1_path_deg=p1['angle_deg'],
            x1_final_m=p1['x_final'], y1_final_m=p1['y_final'],
            length2_m=p2['length_m'], angle2_path_deg=p2['angle_deg'],
            x2_final_m=p2['x_final'], y2_final_m=p2['y_final'],
        )
        for k in ['n_candidates','angle1_deg','angle2_deg','metric1','metric2',
                  'metric2_over_metric1','branch_active','branch_spawned',
                  'share_w1','share_w2','lambda_c1','lambda_c2']:
...
def main():
    ...
    ap.add_argument('root', help='sweep root containing case subdirectories')
