...
V5 corrects the v4 stress-scale mismatch.  The existing sharp-front/CZM engine
...
MODEL_ID = "FEM_CZM_mixed_mode_first_passage_v5_anisotropic_calibrated_tip"
...
@dataclass
class AnisotropicCalibratedTipContext:
...
def main(argv=None):
    ...
    if args.mode != "2d":
        raise SystemExit("v5 anisotropic mixed mode requires --mode 2d")
    if not bool(getattr(args, "crystal_aniso", False)):
        raise SystemExit("v5 requires --crystal-aniso")
    if not bool(getattr(args, "crystal_compete", False)):
        raise SystemExit("v5 requires --crystal-compete")
    if bool(getattr(args, "crystal_branch", False)) or int(getattr(args, "max_fronts", 1)) != 1:
        raise SystemExit("v5 first-passage screen requires branching off and --max-fronts 1")

    context = AnisotropicCalibratedTipContext(
        mm.mixity_loading_angle_deg, mm.target_traction_phase_deg,
    ...
    (out/"anisotropic_calibrated_tip_run_config.json").write_text(json.dumps({
        ...
        "crystal_theta_deg": context.crystal_theta_deg,
        "cleavage_gamma_aniso": context.cleavage_gamma_aniso,
        "note": "anisotropic FEM direction partition + original calibrated sharp-tip stress magnitude",
    }, indent=2))
    ...
    print("MIXED_MODE_V5 complete:", json.dumps(vals, indent=2, default=str))
    return vals
