...
def update_plasticity(
    ...
    * thermo_consistency_mode='time_cone': hazard/time-cone flow.  The local
    ...
    # --- Additive two-branch flow stress (model A): sigma_y = sigma_P + sigma_T(rho) ---
    # sigma_P : Peierls/kink-pair, phi=1, rho-independent (low-T thermal stress).
    # sigma_T : forest/Taylor, phi=1/(2b sqrt(rho)); carries the rho-coupled
    #           hardening->softening (paper Eq. 8).  Both at the same eps_ref so
    #           the branches add as resistances along one glide path.
    eps_ref = max(float(getattr(disl_cfg, 'flow_epsdot_ref', 1e-5)), 1e-30)
    sigma_y, sigma_P, sigma_T = flow_stress_two_branch(
        rho_safe, T, eps_ref, plast_model, disl_cfg, b, return_branches=True)
    ...
    # Kinetic rate at the current stress.  In thermodynamic modes this is the
    # mobility/event clock; in legacy explicit mode it is also the raw rate.
    dot_ep_kin = _kinetic_arrhenius_rate(rho_safe, seq, sigma_P, phi, delta, T,
                                         plast_model, disl_cfg, b)
