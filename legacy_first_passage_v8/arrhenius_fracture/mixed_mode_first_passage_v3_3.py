...
def circular_robust_spread_deg(values: np.ndarray | list[float]) -> float:
    """Robust angular spread using wrapped distances on the circle.

    The previous implementation used a linear median absolute deviation.  That
    falsely reports a huge spread when equivalent annular phases straddle the
    -180/+180 branch cut, as occurred for the raw Mode-I basis.  Candidate
    circular medians are taken from the observed angles; the candidate with
    the smallest median wrapped distance is used.
    """
    ...
    best = float("inf")
    for center in a:
        dist = np.abs((a - float(center) + 180.0) % 360.0 - 180.0)
...
def project_near_tip_modes(mesh, sigma_gp, d, crack_tip, crack_direction,
    ...
    psi_spread = circular_robust_spread_deg(psi_vals) if len(fits)>1 else float("inf")
