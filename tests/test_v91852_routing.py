...
import run_mpz_v9_18_5_2_mode_i_rcurve as mode_runner
import run_mpz_v9_18_5_2_persistent_plastic_wake as campaign_runner


def test_mode_i_command_routes_to_v91852(monkeypatch, tmp_path):
    ...
    cmd = mode_runner._build_command_v91852(
        "python", SimpleNamespace(), "ceramic", {}, 700.0, tmp_path
    )
    assert "arrhenius_fracture.mode_i_first_passage_v9_18_5_2" in cmd
    assert "arrhenius_fracture.mode_i_first_passage_v9_11" not in cmd


def test_campaign_command_routes_to_v91852(monkeypatch, tmp_path):
    ...
    cmd = campaign_runner._build_command_v91852(
        SimpleNamespace(), "ceramic", tmp_path, False
    )
    assert "run_mpz_v9_18_5_2_mode_i_rcurve.py" in cmd
