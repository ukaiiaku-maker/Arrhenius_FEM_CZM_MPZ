#!/usr/bin/env python3
"""One-run-per-condition v3.1 phase-ratio calibrated mixed-mode campaign."""
...
def main():
    ...
    p.add_argument('--T-K',type=float,default=500);p.add_argument('--outroot',default='runs/mixed_mode_fem_czm_v3_1_phase_ratio_500K')
    ...
    cp=subprocess.run([py,'-m','arrhenius_fracture.mixed_mode_first_passage_v3_1','--help'],text=True,capture_output=True)
    if cp.returncode:raise SystemExit(cp.stderr)
    ...
    def run_case(klass,target):
        ...
        cmd=[py,'-m','arrhenius_fracture.mixed_mode_first_passage_v3_1','--mixity-loading-angle-deg',fs(alpha),'--target-mode-phase-deg',fs(target),'--deterministic-threshold','--shear-emission-weight',fs(a.shear_emission_weight),
    ...
    pd.DataFrame(results).to_csv(outroot/'campaign_status_v3_1.csv',index=False)
    good=[r for r in results if r['status']!='failed']
    if good:pd.DataFrame(good).to_csv(outroot/'mixed_mode_v3_1_all_cases.csv',index=False)
