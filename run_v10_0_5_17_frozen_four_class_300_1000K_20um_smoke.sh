...
CAMPAIGN_ROOT=${CAMPAIGN_ROOT:-$ROOT/runs/v10_0_5_17_frozen4_anisotropic_300_1000K_20um_smoke_v1}
...
if [[ ! -f "$FAMILY_JSON" ]]; then
  ...
  echo "Copy the audited family into the FEM repository once before launching." >&2
  exit 1
fi
...
for option in options:
    for temperature in temperatures:
        token = f"v10.0.5.17-frozen-four-class|{base}|{option}|{temperature}".encode()
...
release=10.0.5.17-frozen-four-class
entry=arrhenius_fracture.mode_i_first_passage_v10_0_5_17_four_class_standalone
...
run_case() {
  ...
  local -a CMD
  CMD=(
    python -m arrhenius_fracture.mode_i_first_passage_v10_0_5_17_four_class_standalone
...
echo "Standalone frozen four-class smoke complete: $CAMPAIGN_ROOT"
