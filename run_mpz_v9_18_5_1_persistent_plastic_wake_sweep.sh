...
OUTROOT_BASE=${OUTROOT_BASE:-runs/mpz_v9_18_5_1_safe_target_stop_v1}
TEMPS=${TEMPS:-"700"}
SEEDS=${SEEDS:-"1"}
...
for T in $TEMPS; do
  ...
  mkdir -p "$case_root"
  echo "=== v9.18.5.1 startup-safe target stop: T=${T} K, committed target=${TARGET_EXT_UM} um ==="
  set +e
  ...
  "$PYTHON_BIN" run_mpz_v9_18_5_1_persistent_plastic_wake.py \
