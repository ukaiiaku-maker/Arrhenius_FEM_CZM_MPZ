...
OUTROOT_BASE=${OUTROOT_BASE:-runs/mpz_v9_18_5_7_subsegment_quality_v1}
TEMPS=${TEMPS:-"700"}
SEEDS=${SEEDS:-"1"}
...
for T in $TEMPS; do
  ...
  mkdir -p "$case_root"
  echo "=== v9.18.5.7 subsegment-aware quality certification: T=${T} K, committed target=${TARGET_EXT_UM} um ==="
  set +e
  ...
  "$PYTHON_BIN" run_mpz_v9_18_5_7_persistent_plastic_wake.py \
