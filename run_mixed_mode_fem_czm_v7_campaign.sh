...
CALROOT="${CALROOT:-runs/mixed_mode_fem_czm_v7_full_circle_calibration}"
OUTROOT="${OUTROOT:-runs/mixed_mode_fem_czm_v7_full_circle_500K}"
PSI="${TARGET_PSI:--30 0 30}"
THETA="${CRYSTAL_THETA_DEG:-45}"

if [[ "${RECALIBRATE:-0}" == "1" || ! -f "$CALROOT/mixed_mode_loading_calibration_v7.csv" ]]; then
  conda run -n "$ENV" python calibrate_mixed_mode_loading_v7.py \
    --out "$CALROOT" \
    --target-psi-deg="$PSI" \
    --T-K "${T_K:-500}" \
    --nx "${NX:-24}" --ny "${NY:-48}" \
  ...
  echo "Using existing v7 exact-backend full-circle calibration: $CALROOT/mixed_mode_loading_calibration_v7.csv"
fi

conda run -n "$ENV" python run_mixed_mode_fem_czm_v7_campaign.py \
  --parameter-table "${PARAMETER_TABLE:-four_class_exp_floor_exact_model_inputs.csv}" \
  --calibration-csv "$CALROOT/mixed_mode_loading_calibration_v7.csv" \
...
conda run -n "$ENV" python plot_mixed_mode_fem_czm_v7_results.py --root "$OUTROOT"
