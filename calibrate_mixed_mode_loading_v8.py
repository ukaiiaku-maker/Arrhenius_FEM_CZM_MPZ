#!/usr/bin/env python3
"""Exact adaptive-CZM, full-circle mixed-mode calibration, v8.
...
CALIBRATION_ID = "mixed_mode_fem_czm_v8_exact_backend_boolean_safe_calibration"
...
def bool_col(value):
    """Decode CSV Boolean values without losing numeric truth values.

    pandas may return a Boolean column as bool, numpy.bool_, integer 1, or
    floating-point 1.0 after aggregation.  The v7 parser accepted ``"1"`` but
    rejected ``"1.0"``, causing every valid production-backend phase probe to
    be discarded by the root finder.
    """
    if value is None:
        return False
    ...
    if text in {"true", "t", "yes", "y", "on"}:
        return True
    if text in {"false", "f", "no", "n", "off", "", "none", "nan"}:
        return False
    try:
        number = float(text)
    except ValueError:
    ...
    def evaluate(alpha_unwrapped):
        ...
        rec = {
            **raw,
            ...
            "phase_probe_flag_raw": raw.get("traction_phase_probe_reliable", raw.get("traction_probe_reliable")),
            "phase_probe_flag_decoded": bool_col(raw.get("traction_phase_probe_reliable", raw.get("traction_probe_reliable", False))),
...
def main():
    ...
    p.add_argument("--out", default="runs/mixed_mode_fem_czm_v8_backend_calibration")
    ...
    csv_path = out / "mixed_mode_loading_calibration_v8.csv"
    with csv_path.open("w", newline="") as fp:
        cols = sorted({k for r in rows for k in r})
        w = csv.DictWriter(fp, fieldnames=cols)
        w.writeheader(); w.writerows(rows)
    pd.DataFrame(all_hist).to_csv(out / "mixed_mode_loading_calibration_history_v8.csv", index=False)
    (out / "production_backend_basis_v8.json").write_text(json.dumps({
        "calibration_id": CALIBRATION_ID,
        "model": MODEL_ID,
    ...
    if bad and not a.force:
        raise SystemExit("v8 exact-backend full-circle calibration failed: " +
