...
RUNNER="$ROOT/run_v10_0_5_18_3_2_four_class_joint_K_single_trial_focused_100um.sh"

if [[ ! -f "$RUNNER" ]]; then
  echo "ERROR: missing v10.0.5.18.3.2 single-trial runner: $RUNNER" >&2
  exit 1
fi
...
if [[ -z "${CAMPAIGN_ROOT:-}" ]]; then
  ...
  CAMPAIGN_ROOT="$ROOT/runs/v10_0_5_18_3_2_four_class_14T_${TARGET_TAG}um_theta${THETA}_joint_K_single_trial_v1"
fi

if pgrep -f 'python.*mode_i_first_passage_v10_0_5_18_3_2' >/dev/null; then
  echo "ERROR: another v10.0.5.18.3.2 solver is active" >&2
  ps -axo pid,ppid,etime,%cpu,%mem,state,command |
    grep '[m]ode_i_first_passage_v10_0_5_18_3_2' >&2 || true
  exit 1
fi
...
runner=run_v10_0_5_18_3_2_four_class_joint_K_single_trial_focused_100um.sh
sweep_wrapper=run_v10_0_5_18_3_2_four_class_14T_joint_K_single_trial_sweep.sh
