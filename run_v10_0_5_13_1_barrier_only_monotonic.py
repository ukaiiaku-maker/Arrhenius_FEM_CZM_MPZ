#!/usr/bin/env python3
"""v10.0.5.13.1 campaign wrapper for preserved 2-D state configuration."""
from __future__ import annotations

import shlex
import subprocess

import run_v10_0_5_13_barrier_only_monotonic as _base
...
def _build_command(py, args, option_key, T_K, target_um, case_dir):
    ...
    old = "arrhenius_fracture.mode_i_first_passage_v10_0_5_13_barrier_only"
    try:
        cmd[cmd.index(old)] = ENTRY_MODULE
    except ValueError as exc:
        raise RuntimeError(f"v10.0.5.13 command lacks expected entry {old}") from exc
    return cmd
...
def _preflight(py: str, run_tests: bool):
    ...
    compile_cmd = [
        ...
        "-m",
        "py_compile",
        "arrhenius_fracture/mode_i_first_passage_v10_0_5_13_1_barrier_only.py",
        "run_v10_0_5_13_1_barrier_only_monotonic.py",
    ]
    cp = subprocess.run(compile_cmd, text=True)
    if cp.returncode != 0:
        raise SystemExit(f"v10.0.5.13.1 compile failed: {shlex.join(compile_cmd)}")
    if run_tests:
        test_cmd = [
            py,
            "-m",
        ...
        if cp.returncode != 0:
            raise SystemExit(f"v10.0.5.13.1 tests failed: {shlex.join(test_cmd)}")
