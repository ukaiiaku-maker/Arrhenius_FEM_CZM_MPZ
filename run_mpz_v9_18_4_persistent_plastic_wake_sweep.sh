...
OUTROOT_BASE=${OUTROOT_BASE:-runs/mpz_v9_18_4_mechanically_valid_sweep_v1}
TEMPS=${TEMPS:-"700"}
SEEDS=${SEEDS:-"1"}
...
EDGE_FRONT_REGULARIZATION_ANGLES_DEG=${EDGE_FRONT_REGULARIZATION_ANGLES_DEG:-"0.01 0.025 0.05 0.1 0.2 0.35"}
...
for T in $TEMPS; do
  ...
  mkdir -p "$case_root"
  echo "=== v9.18.4 mechanically valid front: T=${T} K, committed target=${TARGET_EXT_UM} um ==="
  set +e
  ...
  ARRHENIUS_EDGE_FRONT_REGULARIZATION_ANGLES_DEG="$EDGE_FRONT_REGULARIZATION_ANGLES_DEG" \
  ...
  "$PYTHON_BIN" run_mpz_v9_18_4_persistent_plastic_wake.py \
