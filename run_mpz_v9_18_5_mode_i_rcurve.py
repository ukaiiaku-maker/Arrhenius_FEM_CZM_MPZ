#!/usr/bin/env python3
"""Select the bounded v9.18.5 target-stop/quality-corridor Mode-I entry point."""
...
def _build_command_v9185(py, args, class_name, manifest, T_K, case_dir):
    cmd = _original_build_command(py, args, class_name, manifest, T_K, case_dir)
    old = "arrhenius_fracture.mode_i_first_passage_v9_11"
    new = "arrhenius_fracture.mode_i_first_passage_v9_18_5_bounded"
    try:
...
def main():
    ...
    _base.build_command = _build_command_v9185
    try:
        return _base.main()
    finally:
