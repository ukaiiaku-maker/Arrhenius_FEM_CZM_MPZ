#!/usr/bin/env python3
"""v10.0.5.13.5 campaign wrapper for the 100 um corridor repair."""
from __future__ import annotations

import shlex
import subprocess

import run_v10_0_5_13_4_barrier_only_monotonic as _base

POINT_RELEASE = "10.0.5.13.5"
ENTRY_MODULE = (
    "arrhenius_fracture.mode_i_first_passage_v10_0_5_13_5_barrier_only"
)
...
def _build_command(py, args, option_key, T_K, target_um, case_dir):
    ...
    old = "arrhenius_fracture.mode_i_first_passage_v10_0_5_13_4_barrier_only"
    try:
        cmd[cmd.index(old)] = ENTRY_MODULE
    except ValueError as exc:
        raise RuntimeError(f"v10.0.5.13.4 command lacks expected entry {old}") from exc
    mode_index = cmd.index("--bulk-plasticity-mode") + 1
    if cmd[mode_index] != "tip_only":
        raise RuntimeError(
            "v10.0.5.13.5 command did not retain tip_only plasticity scope"
        )
    return cmd
...
def _preflight(py: str, run_tests: bool):
    ...
    compile_cmd = [
        ...
        "-m",
        "py_compile",
        "arrhenius_fracture/mode_i_first_passage_v10_0_5_13_5_barrier_only.py",
        "run_v10_0_5_13_5_barrier_only_monotonic.py",
    ]
    cp = subprocess.run(compile_cmd, text=True)
    if cp.returncode != 0:
        raise SystemExit(
            f"v10.0.5.13.5 compile failed: {shlex.join(compile_cmd)}"
        )
    if run_tests:
        test_cmd = [
            py,
        ...
        if cp.returncode != 0:
            raise SystemExit(
                f"v10.0.5.13.5 tests failed: {shlex.join(test_cmd)}"
            )
