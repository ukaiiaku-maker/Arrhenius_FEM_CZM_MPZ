...
# BASE_DU/BASE_DT. DU and DT may be coarsened together; the adaptive event
# controller reduces both by the same accepted trial fraction near events.
...
case "$MODE" in
  smoke)
    DEFAULT_OUTROOT="$ROOT_DIR/runs/v10_0_5_13_4_tip_only_DBTT_700K_20um_macro100_v1"
    ;;
  full)
    DEFAULT_OUTROOT="$ROOT_DIR/runs/v10_0_5_13_4_tip_only_4class_300_1200K_100um_macro100_v1"
    ;;
  *)
...
args=(
  "$PYTHON_BIN" "$ROOT_DIR/run_v10_0_5_13_4_barrier_only_monotonic.py"
  --mode "$MODE"
...
printf 'Python: %s\n' "$PYTHON_BIN"
printf 'Output: %s\n' "$OUTROOT"
printf 'Point release: 10.0.5.13.4\n'
printf 'Plasticity scope: tip_only (elastic bulk + moving crack-tip MPZ)\n'
printf 'Policy propagation: tip_only enforced through v10.0.5.13 core entry\n'
printf 'Ramp audit: %s\n' "$RATE_AUDIT"
printf 'Launching:'
printf ' %q' "${args[@]}"
printf '\n'
