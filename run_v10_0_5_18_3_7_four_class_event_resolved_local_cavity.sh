...
TMP=$(mktemp "${TMPDIR:-/tmp}/v10051837_event_resolved_runner.XXXXXX")
trap 'rm -f "$TMP"' EXIT

sed \
  -e 's/mode_i_first_passage_v10_0_5_18_3_4_four_class_path_aware_growth_envelope/mode_i_first_passage_v10_0_5_18_3_7_event_resolved_local_cavity/g' \
  -e 's/persistent_site_production_manifest_v10_0_5_18_3_4.json/persistent_site_production_manifest_v10_0_5_18_3_7.json/g' \
  -e 's/release=10.0.5.18.3.4/release=10.0.5.18.3.7/g' \
  -e 's/Focused v10.0.5.18.3.4 path-envelope validation complete/Focused v10.0.5.18.3.7 event-resolved local-cavity validation complete/g' \
  "$BASE_RUNNER" > "$TMP"
